...
public class UprawnieniaDAO{
    ...
    final String ADD_PERMISSION = "INSERT INTO Permissions (ID, admin, add_user, del_user, edit_user, finance_management, show_d_data) VALUES (?,?,?,?,?,?,?)";
    final String EDIT_PERMISSION =  "UPDATE Permissions SET admin = ?, add_user = ?, del_user = ?, edit_user = ?, finance_management = ?, show_d_data = ? WHERE ID = ?";
    ...
    public void addPermission(int id, int admin, int add_user, int del_user, int edit_user, int finance_management, int show_d_data){
        baza1.update(ADD_PERMISSION, id, admin, add_user, del_user, edit_user, finance_management, show_d_data);
    }

    public void editPermission(int id, int admin, int add_user, int del_user, int edit_user, int finance_management, int show_d_data){
        baza1.update(EDIT_PERMISSION, admin, add_user, del_user, edit_user, finance_management, show_d_data, id);
    }
    ...
    private RowMapper<Uprawnienia> getMap() {
		RowMapper<Uprawnienia> Map = (rs, rowNum) -> {
            ...
            upr.setFinance_management(rs.getBoolean("finance_management"));
